...
const App = {
  ...
  getCurrentTraining() {
    ...
    if (w >= PROGRAM.length) return null;
    // Find next training day
    const week = PROGRAM[w];
    for (let dd = d; dd < week.days.length; dd++) {
      if (week.days[dd].type === 'train') return { week: w, day: dd, data: week.days[dd] };
    }
    // Next week
    if (w + 1 < PROGRAM.length) {
      for (let dd = 0; dd < PROGRAM[w + 1].days.length; dd++) {
        if (PROGRAM[w + 1].days[dd].type === 'train') return { week: w + 1, day: dd, data: PROGRAM[w + 1].days[dd] };
      }
    }
    return null;
  },
  ...
  renderJourney() {
    const trainingDays = PROGRAM.reduce((sum, week) => sum + week.days.filter(day => day.type === 'train').length, 0);
    const completed = Math.min(trainingDays, (state.completedDays || []).length);
    const pct = trainingDays ? Math.round((completed / trainingDays) * 100) : 0;
    ...
    if (label) label.textContent = pct + '% шляху';
  },
  ...
  renderProgram() {
    ...
    container.innerHTML = PROGRAM.map((week, wi) => {
      const isCurrentWeek = wi === state.currentWeek;
      const isPastWeek = wi < state.currentWeek;
      const status = isPastWeek ? '✅' : isCurrentWeek ? '⚡' : '🔒';

      const days = week.days.map((d, di) => {
        const key = `${wi}-${di}`;
        const isDone = state.completedDays.includes(key);
        const isCurrent = wi === state.currentWeek && di === state.currentDay;
        const cls = isDone ? 'completed' : isCurrent ? 'current' : '';
        const icon = d.type === 'rest' ? '😴' : isDone ? '✅' : isCurrent ? '🎯' : '🏋️';
        const check = isDone ? '✅' : isCurrent ? '◀' : '';

        return `<div class="day-row ${cls}">
          <span class="day-icon">${icon}</span>
          <span class="day-label">День ${di + 1}</span>
          <span class="day-desc">${d.title}${d.type === 'train' && d.sets ? ` · ${d.sets.length} підходи` : ''}</span>
          <span class="day-check">${check}</span>
        </div>`;
      }).join('');

      return `<div class="week-block ${isCurrentWeek ? 'open' : ''}">
        <div class="week-header" onclick="this.parentElement.classList.toggle('open')">
          <span class="week-number">Тиждень ${wi + 1}</span>
  ...
  initReminderSettings() {
    ...
    this.renderReminderStatus(enabled ? 'Нагадування увімкнено.' : 'Нагадування вимкнено.');
  ...
  updateReminderSettings() {
    ...
    localStorage.setItem('pullup_reminder_time', time);
    ...
    NativeBridge.send('scheduleReminder', { hour: parts[0] || 18, minute: parts[1] || 0 });
    this.renderReminderStatus('Запитуємо дозвіл на нагадування…');
  },
  ...
  onReminderResult(result) {
    ...
    if (result.status === 'scheduled') {
      this.renderReminderStatus(`Нагадування заплановано на ${result.time || '18:00'}.`);
  ...
  finishWorkout() {
    ...
    // Advance program
    this.advanceProgram(t.week, t.day);
    ...
    saveState();
    NativeBridge.send('workoutCompleted');
  ...
  showCelebration(xp) {
    ...
    document.getElementById('cel-sub').textContent = m.s;
