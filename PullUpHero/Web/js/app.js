...
const ADAPTIVE_LEVELS = [
  { level: 1, title: "Рівень 1 · База героя 🌱", sets: 3, reps: 2, rest: 90, xp: 45, note: "Спокійно виконай 3 × 2 з резинкою. Якість важливіша за кількість." },
  { level: 2, title: "Рівень 2 · Більше сили 💪", sets: 3, reps: 3, rest: 90, xp: 55, note: "Додаємо лише 1 повтор у підхід. Роби без ривків." },
  { level: 3, title: "Рівень 3 · Стійкий герой ⚡", sets: 4, reps: 3, rest: 105, xp: 65, note: "Додаємо ще один підхід, але темп залишається спокійним." },
  { level: 4, title: "Рівень 4 · Сильні руки 🔥", sets: 4, reps: 4, rest: 120, xp: 80, note: "Працюй акуратно. Якщо важко — це нормально." },
  { level: 5, title: "Рівень 5 · Майстер турніка 🏆", sets: 5, reps: 4, rest: 120, xp: 100, note: "Фінальна місія з резинкою. Далі можна думати про легшу резинку." }
];
...
const App = {
  ...
  renderToday() {
    ...
    if (this.hasCompletedTaskToday()) {
      ...
      document.getElementById('today-sets').innerHTML = `<p style="color:var(--text2);text-align:center;padding:1rem;line-height:1.55;">Супер, ${PLAYER_NAME_VOCATIVE}!<br>Наступне завдання відкриється завтра 🌙</p>`;
      const note = document.getElementById('today-note');
      if (note) note.textContent = 'Сьогодні відпочивай, грайся та набирайся сил для нової місії.';
      document.getElementById('btn-train').style.display = 'none';
      return;
    }
    ...
    const note = document.getElementById('today-note');
    if (note) note.textContent = t.data.note || 'Роби рухи плавно і з дорослим поруч.';

    const sets = t.data.sets || [];
    const progressText = `Рівень ${state.trainingLevel}: ${state.levelSuccessCount || 0}/${ADAPTIVE_SUCCESS_TO_LEVEL_UP} успішних тренувань до наступного рівня`;
    const html = `
      <div class="adaptive-status">
        ...
        ${state.consecutiveMisses ? `<div class="adaptive-status-warn">Складний період: ${state.consecutiveMisses}/${ADAPTIVE_FAILURES_TO_EASE} спроб, після яких місія стане легшою.</div>` : ''}
      </div>
      ${sets.map((s, i) => `
    ...
    container.innerHTML = `
      <div class="adaptive-program-summary">
        ...
        <div class="adaptive-program-text">
          Поточна місія підлаштовується під результат. Для переходу далі потрібно ${ADAPTIVE_SUCCESS_TO_LEVEL_UP} успішних тренувань на рівні.
          Якщо 2 рази поспіль не вийде — програма сама трохи полегшить завдання.
        </div>
      </div>
  ...
  updateReminderSettings() {
    ...
    if (!NativeBridge.available()) {
      this.renderReminderStatus('Нагадування доступне у застосунку для iPhone та iPad.');
      return;
    }
    if (!enabled) {
    ...
    this.renderReminderStatus(`Запитуємо дозвіл на нагадування на ${reminder.time}…`);
  },
  ...
  onReminderResult(result) {
    ...
    if (result.status === 'scheduled') {
      ...
      this.renderReminderStatus('Дозвіл не надано. Увімкнути сповіщення можна в налаштуваннях iOS.');
    ...
    if (isSuccessful) {
      ...
      if (state.levelSuccessCount >= ADAPTIVE_SUCCESS_TO_LEVEL_UP) {
        if (state.trainingLevel < ADAPTIVE_LEVELS.length) {
          ...
          adaptiveMessage = `Новий рівень! Тепер місія стане трохи сильнішою 💪`;
        } else {
          state.levelSuccessCount = ADAPTIVE_SUCCESS_TO_LEVEL_UP;
          adaptiveMessage = `Фінальний рівень закріплено 🏆`;
        }
      } else {
        adaptiveMessage = `До наступного рівня ще ${ADAPTIVE_SUCCESS_TO_LEVEL_UP - state.levelSuccessCount} успішних тренувань.`;
      }
    } else {
      ...
      if (state.consecutiveMisses >= ADAPTIVE_FAILURES_TO_EASE) {
        if (state.trainingLevel > 1) {
          state.trainingLevel -= 1;
          state.levelSuccessCount = 0;
          adaptiveMessage = `Справжні герої не здаються. Полегшуємо місію на один рівень і пробуємо ще раз 🌱`;
        } else {
          state.levelSuccessCount = 0;
          adaptiveMessage = `Залишаємось на базовому рівні. Завтра спробуємо спокійніше 🌱`;
        }
        state.consecutiveMisses = 0;
      } else {
        adaptiveMessage = `Нічого страшного. Сьогодні просто записали спробу.`;
      }
    }
